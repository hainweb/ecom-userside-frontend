...
export const generateInvoice = (orderTrack, products, companyInfo = {}) => {
  try {
    ...
    const formatDate = (str) => {
      if (!str) return "N/A";
      // remove the " at " so Date.parse can handle it
    ...
    const checkPageBreak = (currentY, requiredSpace = 20) => {
      ...
      if (currentY + requiredSpace > pageH - 40) { // 40 for footer space
        doc.addPage();
    ...
    const company = {
      ...
      address: String(companyInfo.address || "123 Business Street, City, State 12345"),
    ...
    safeText(`INV-${order._id?.slice(-8) || "N/A"}`, pageW - 20, 42, { align: "right" });
    safeText(`${order._id?.slice(-12) || "N/A"}`, pageW - 20, 48, { align: "right" });
    ...
    // Full address line: address, city, state – pincode
    safeText(
    ...
    const timeline = [
      { cond: order.status, label: "Order Placed", date: order.date, color: [34, 197, 94] },
      { cond: order.status2, label: "Order Shipped", date: order.shipedDate, color: [59, 130, 246] },
      { cond: order.status3, label: "Order Delivered", date: order.deliveredDate, color: [34, 197, 94] },
      { cond: order.cancel, label: "Order Cancelled", date: order.canceledTime, color: [239, 68, 68] },
    ];
    ...
    if (couponDiscount > 0) totals.push(["Coupon Discount", `- Rs. ${formatCurrency(couponDiscount)}`]);
    if (taxAmount > 0)      totals.push([`Tax (${taxRate}%)`, `Rs. ${formatCurrency(taxAmount)}`]);
    if (shippingFee > 0)    totals.push(["Shipping Fee", `Rs. ${formatCurrency(shippingFee)}`]);
    ...
    doc.autoTable({
      ...
      didParseCell: (data) => {
        ...
        if (data.cell.text[0]?.includes("Coupon Discount") || data.cell.text[0]?.includes("- Rs.")) {
    ...
    const terms = [
      ...
      "5. For support, contact us at " + company.email + " or " + company.phone + "."
    ];
    
    let currentTermY = termsY + 15;
    terms.forEach((term, index) => {
      // Split long terms if needed
    ...
    doc.setFont("helvetica", "normal");
    doc.setFontSize(8);
    safeText("For any queries, please contact our customer support team.", margin, footerY + 12);

    // Footer right side
    doc.setTextColor(255, 255, 255);
    safeText(
      `Generated: ${formatDate(new Date().toString())} ${new Date().toLocaleTimeString()}`,
      pageW - margin,
      footerY + 5,
      { align: "right" }
    );
    ...
    // ——— Save PDF ———
    const fileName = `Invoice_${order._id?.slice(-8) || "Order"}_${formatDate(order.date).replace(/[^a-zA-Z0-9]/g, "-")}.pdf`;
    doc.save(fileName);
...
const calculateSubtotal = (products) =>
  products.reduce((sum, p) => sum + (p.product?.Price || 0) * (p.quantity || 1), 0);